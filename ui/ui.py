    ...
    def plugins(self, data):
        ...
        if "channels" in data:
            with open(config_file, "w", newline="") as file:
                file.write(json.dumps(data["channels"], indent=4))
        else:
                ...
                try:
                    ...
                    if hasattr(job.job_func, "args") and job.job_func.args:
                        ...
                        if isinstance(command_args, list) and len(command_args) > 1:
                            ...
                            if (
                                plugin_name not in next_executions
                                or next_run < next_executions[plugin_name]
                            ):
    ...
    def handle_command(self, command):
        ...
        try:
            if len(secure_command) > 2 and secure_command[2] == "cli.py":
                ...
                Ui.is_running = False
                emit(
                    "command_completed", {"data": "Comando completado"}, broadcast=True
                )
                ...
                self.handle_output(
                    "only python cli.py command can be executed from here."
                )
                Ui.is_running = False
                emit(
                    "command_completed", {"data": "Comando completado"}, broadcast=True
