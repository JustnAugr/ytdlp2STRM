...
def fix_vtt_alignment(vtt_text):
    ...
    for block in cue_blocks:
        ...
        text_lines = [
            clean_vtt_text_line(line) for line in block_lines[timing_idx + 1 :]
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert existing WebVTT subtitles to SRT sidecar files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--delete-vtt",
        action="store_true",
        help="Delete .vtt files after successful conversion",
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Show skipped and empty files"
    )
